function mni_copyfile(source, dest, varargin)
% Copy file(s)
...
for i=1:numel(source)
    ...
    if opts.nifti
        if strcmp(spm_file(source{i},'ext'),'img')
            sts = copyfile(spm_file(source{i},'ext','hdr'), spm_file(dest{i},'ext','hdr'), opts.mode{:});
            sts = copyfile(spm_file(source{i},'ext','mat'), spm_file(dest{i},'ext','mat'), opts.mode{:});
            sts = copyfile(spm_file(source{i},'ext','json'), spm_file(dest{i},'ext','json'), opts.mode{:});
        elseif strcmp(spm_file(source{i},'ext'),'hdr')
            sts = copyfile(spm_file(source{i},'ext','img'), spm_file(dest{i},'ext','img'), opts.mode{:});
            sts = copyfile(spm_file(source{i},'ext','mat'), spm_file(dest{i},'ext','mat'), opts.mode{:});
            sts = copyfile(spm_file(source{i},'ext','json'), spm_file(dest{i},'ext','json'), opts.mode{:});
